...
class Automata {
    ...
    realizarAccion() {
        switch (this.currentSt) {
            case 4:
                console.log(this.cadena + "Numeric constant");
                this.currentSt = 0;
                break;
            case 6:
                console.log(this.cadena + "Invalid numeric string");
                this.currentSt = 0;
                break;
            case 8:
                console.log(this.cadena + "Invalid string");
                this.currentSt = 0;
                break;
            case 14:
                ...
                break;
            case 18:
                console.log(this.cadena + "Identifier");
                this.currentSt = 0;
                break;
        }
    }
...
async function ejecutarAutomata() {
    try {
        const datosMatriz = await leerArchivo('matrix.txt');
        const lineas = datosMatriz.split('\n');
        const matriz = lineas.map(linea => linea.split('\t').map(Number));
        const datosPrueba = await leerArchivo('test.txt');
